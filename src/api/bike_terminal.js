...
export const GetTerminal = async () => {
    try {
        const data = await axios.get('http://localhost:5005/v1/bike-terminal', customConfig)
...
export const GetTerminalById = async (id) => {
    try {
        const data = await axios.get(`http://localhost:5005/v1/bike-terminal/${id}`, customConfig)
        if (data)return data.data
    } catch (err) {
        return null
    }
...
export const UpdateTerminal = async (id, data) => {
    try {
        const response = await axios.put(`http://localhost:5005/v1/bike-terminal/${id}`, data, customConfig)
